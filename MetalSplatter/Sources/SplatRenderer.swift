...
//                let startTime = Date()
                ...
                guard let distanceBuffer = device.makeBuffer(
                    ...
                    print("Failed to create distance buffer.")
                    self.sorting = false
                    return
                }
                ...
                guard let indexOutputBuffer = device.makeBuffer(
                    ...
                    print("Failed to create output indices buffer.")
                    self.sorting = false
                    return
                }
                ...
                guard let commandQueue = device.makeCommandQueue() else {
                    print("Failed to create command queue for MPSArgSort.")
                    self.sorting = false
                    return
                }
                ...
                do {
                    ...
                    print("Failed to set capacity or reorder: \(error)")
                }

//                let elapsed = Date().timeIntervalSince(startTime)
//                print("Sort time (GPU): \(elapsed) seconds")
...
//                print("Sort time (CPU): \(elapsedCPU) seconds")
